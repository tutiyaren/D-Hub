...
class PoliticsController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($keyword) {
            $debates = Debate::where('genre_id', 1)->orderBy('created_at', 'desc')->titleSearch($keyword)->paginate(5);
        }
        if (!$keyword) {
            $debates = Debate::where('genre_id', 1)->orderBy('created_at', 'desc')->paginate(5);
        }
        $genre = Genre::find(1);
        $userId = auth()->user()->id;
        ...
        foreach ($debates as $debate) {
            if (!$anonymity) {
                return view('politics.index', compact('debates', 'genre', 'isBookmarked', 'isVote', 'anonymity'));
            }
    ...
    public function show($id)
    {
        ...
        $userId = auth()->user()->id;
        $anonymity = Anonymity::getByUserId($userId);
        return view('politics.show', compact('debate', 'genre', 'comments', 'anonymity'));
    }

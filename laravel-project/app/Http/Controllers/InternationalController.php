...
class InternationalController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($keyword) {
            $debates = Debate::where('genre_id', 3)->titleSearch($keyword)->orderBy('created_at', 'desc')->paginate(5);
        }
        if (!$keyword) {
            $debates = Debate::where('genre_id', 3)->orderBy('created_at', 'desc')->paginate(5);
        }
        $genre = Genre::find(3);
        $userId = auth()->user()->id;
        ...
        return view('international.index', compact('debates','genre', 'isBookmarked'));
    }
